    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if(sort){
                ...
                const allSort =  await Permission.find().limit(limit).skip(page * limit).sort({ objectSort })
                resolve({
                    status: 'OK',
                    message: 'Get all success',
                    data: allSort,
            ...
            const all =  await Permission.find().limit(limit).skip(page * limit)
